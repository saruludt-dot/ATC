...
if uploaded_file and calculate:
    ...
    # ----------- LIVE ANALYSIS (SEE-SAW BASED) -----------

    if 'atm_strike' in locals():

        st.markdown("## 📈 Live Analysis (See-Saw Based)")
        ...
        ce_df_chart = df[
            (df["Expiry Date"] == expiry_str) &
            (df["Option Type"] == "CE") &
            (df["Strike Price"] == ce_strike)
        ]

        pe_df_chart = df[
            (df["Expiry Date"] == expiry_str) &
            (df["Option Type"] == "PE") &
            (df["Strike Price"] == pe_strike)
        ]
        ...
        # ---------------- CE CHART ----------------
        with col1:
            st.markdown(f"### 🟢 CE Chart ({int(ce_strike)})")

            if not ce_df_chart.empty:

                fig_ce = go.Figure(data=[go.Candlestick(
                    open=ce_df_chart["Close Price"],
                    high=ce_df_chart["High Price"],
                    low=ce_df_chart["Low Price"],
                    close=ce_df_chart["Close Price"]
                )])
        ...
        # ---------------- PE CHART ----------------
        with col2:
            st.markdown(f"### 🔴 PE Chart ({int(pe_strike)})")

            if not pe_df_chart.empty:

                fig_pe = go.Figure(data=[go.Candlestick(
                    open=pe_df_chart["Close Price"],
                    high=pe_df_chart["High Price"],
                    low=pe_df_chart["Low Price"],
                    close=pe_df_chart["Close Price"]
                )])
        ...
        # ---------------- SIGNAL ----------------
        if not ce_df_chart.empty and not pe_df_chart.empty:

            ce_last = ce_df_chart.iloc[-1]["Close Price"]
            pe_last = pe_df_chart.iloc[-1]["Close Price"]

            st.markdown("### 📊 Signal")

            if ce_last > pe_last:
                st.success("🟢 Bullish Trend (CE Strong)")
            elif pe_last > ce_last:
                st.error("🔴 Bearish Trend (PE Strong)")
            else:
